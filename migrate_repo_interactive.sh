...
echo
read -p "👉 Both repositories look good. Proceed with migration? (y/n): " confirm
if [[ "$confirm" != "y" ]]; then
  echo "❌ Aborted by user."
  exit 1
fi
...
echo
read -p "👉 Clone repository from OLD account ($OLD_USER)? (y/n): " confirm
if [[ "$confirm" != "y" ]]; then
  echo "❌ Clone step skipped. Exiting."
  exit 1
fi
...
read -p "👉 Continue and PUSH to NEW account ($NEW_USER)? (y/n): " confirm
if [[ "$confirm" != "y" ]]; then
  echo "❌ Push cancelled. Repository not modified."
  exit 1
fi
